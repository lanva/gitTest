...
(function () {
    ...
    UploadImage.prototype = {
        ...
        getInsertList: function () {
            var i, data, list = [],
                align = getAlign(),
                prefix = editor.getOpt('imageUrlPrefix'),subs,subUrl;
            //修改源码部分
           
            var ueImgUrl = $("#ueImgUrl").val();
            for (i = 0; i < this.imageList.length; i++) {
                data = this.imageList[i];
                subs = data.url;
                subUrl = subs.substring(subs.lastIndexOf("/",subs.lastIndexOf("/")-1));
                list.push({
                    src: ueImgUrl + prefix + subUrl,
                    _src: ueImgUrl + prefix + subUrl,
                    title: data.title,
                    alt: data.original,
                    floatStyle: align
                });
